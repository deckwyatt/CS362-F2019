...
int tribute_funct(struct gameState* state, int currentPlayer, int handPos, int nextPlayer, int tributeRevealedCards[]) {
    ...
    if (state->deckCount[nextPlayer] < cardsToReveal) {
        //reshuffle
        for (i = state->discardCount[nextPlayer] - 1; i >= 0; i--) {
            state->deck[nextPlayer][state->deckCount[nextPlayer]++] = state->discard[nextPlayer][i];//Move to deck
...
int main() {
    ...
    result = tribute_funct(state, 0, 0, 1, tributeRevealedCards);
